...
const ProfileOptions: React.FC<{
    Arrayfunction?: Array<() => void>;
    className?: string;
    disabled?: boolean;
}> = ({ Arrayfunction, className, disabled }) => {
    return (
        <div className=" m-0 p-0 min-h-0 w-full h-min-content mt-[70px]">
            <div className="lg:-ml-12 lg:-mr-12 py-0 lg:px-24 flex flex-row items-center justify-start gap-6 overflow-auto font-semibold">
                ...
                <button onClick={Arrayfunction![0]} className="block outline-none border-none p-0 m-0 bg-none text-gray-400 lg:text-lg transition-all duration-150 ease-in-out cursor-pointer hover:text-[#6BA6FE]">
                    Meeting
                </button>
